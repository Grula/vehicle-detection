...
\mentor{dr Milena \textsc{Vujosevic Janičić}, vandredni profesor\\ Univerzitet u Beogradu, Matematički fakultet}
...
% uvod o masinskom
Mašinsko učenje (eng.~\textit{machine learning}) se odnosi na sposobnost mašine da donosi zaključke koristeći dostupne podatke umesto strogo definisanih i kodiranih pravila. Zapravo, mašinsko učenje omogućava računarima da sami uče. %Ova vrsta učenja koristi moć savremenih računara, koji lako mogu obraditi velike skupove i količine različitih podataka.
...
Model \textit{StyleGAN} generiše fotorealistične slike. Tokom razvoja, model \textit{StyleGAN} je prošao kroz fazu optimizacije i manje izmene mreže sinteze. Nova verzija \textit{StyleGAN}-a, \textit{StyleGAN2} donosi i do 60\% brže obučavanje modela u odnosu na prvu verziju \cite{karras2020analyzing}. Zbog ovakvih ubrzanja, dirketno se smanjuje procesorska snaga potrebna za obučavanje. U radu se koristi ova poboljšana verzija, tačnije arhitektura \textit{StyleGAN2}. % Model \textit{StyleGAN2} proizvodi sintetičke slike polazeći od niske rezolucije (4x4), povećavajući je do visoke rezolucije (1024×1024) \cite{karras2020analyzing}.
...
U okviru implementacije, dostupan skup podataka se proširuje metodama augmentacije i sintetičkim podacima koje generiše model \textit{StlyeGAN2}. %Generisanje slika se vrši dok se ne dostigne balans između svih klasa. 
...
% Moglo je da podstakne generator da generiše jednu ili mali skup slika. Takav problem se naziva kolaps režima (eng.~\textit{mode collapse}). Iz rezultata se zaključuje da se taj problem ne manifuestuje jer se generiše više raziličith slika sa izmešanim stilovima. 
...
% \item Izlaz modela se račvao na dve nove duboko povezane mreže dubine četiri. Obe mreže imaju izlaz od po četiri neurona. Prvi izlaz će predstavljati predviđenu klasu, dok drugi koordinate predviđenih okvira.
% \item Poslednji sloj prve mreže ima aktivacionu funkciju \textit{softmax}. Izlaz aktivacione funkcije može da se tumači kao dodeljivanje ocene pouzdanosti klasama objekata.
% \item Poslednji sloj druge mreže ima \textit{sigmoid}-nu aktivacionu funkciju koja daje četiri relativne koordinate graničnog okvira.
\end{itemize}
...
Kako model VGG16 ne može da d\^{a} zadovoljavajuće rezultate, u okviru rada razmatra se i model za detekciju i predviđanje objekata YOLO. Koriste se dve konfiguracije modela YOLO. Prva konfiguracija modela YOLO služi za poređenje rezultata, i u mogućnosti je da predvidi do osamdeset klasa. Druga konfiguracija modela YOLO je u stanju da predvidi do četiri klase.%  Količina podataka koja je na raspolaganju nije dovoljna za obučavanje modela YOLO . Obučavanje modela na tom skupu podataka davalo bi rezultate sa visokom pristrasnošću (eng.~\textit{bias}).

% \begin{figure}[!ht]
%     \centering
%     \includegraphics[width=1\textwidth]{matfmaster/glava4/yolo4speed.png}
%     \caption{Poredak brzina yolov4 sa drugim modelima}
%     \label{fig:section4_yolo4_graph}
% \end{figure}
...
% Klasa \textit{motorbike} se predviđa u 40\% slučajeva dok se klasa \textit{bus} ne predviđa.

% Iz rezultata prikazanih u tabelama \ref{tab:section4_yolo4base_confusionmatrix} i \ref{tab:section4_yolo4base_results} može se zaključiti da su rezultati adekvatni. Preciznost klasa u odnosu na IoU prag je prikazana na slici \ref{fig:section4_yolo4base_prc}. Problem nastaje kod predviđanja klase \textit{car} kod koje neki objekti imaju slične karateristike kao klasa \textit{truck}. Problem sličnih karakteristika između dve klase su prikazani na slici \ref{fig:section4_yolo4base_problemimages}.
